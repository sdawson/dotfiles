...
# pmarreck's relative sourcing bash functions
# Source: https://github.com/pmarreck/dotfiles/blob/master/bin/functions/source_relative.bash
#
#!/usr/bin/env bash
...
source_relative_once() {
  ...
  [[ -v _TRACE_SOURCING ]] && echo "source_relative_once invocation #${_debug_id}: prior to sourcing, _SOURCED_FILES is now $(wc -w <<< "$_SOURCED_FILES") long" >&2
  ...
  [[ -v _TRACE_SOURCING ]] && echo "source_relative_once invocation #${_debug_id}: after sourcing \"$_abs_path\", _SOURCED_FILES is now $(wc -w <<< "$_SOURCED_FILES") long" >&2
